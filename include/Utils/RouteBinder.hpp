...
{
private:
    static constexpr crow::HTTPMethod toCrowMethod(protocol::HttpMethod m)
    {
    ...
    static PathParams makePathParams(Args... args)
    {
        static_assert(sizeof...(Args) == boost::pfr::tuple_size_v<PathParams>, "Path parameter count mismatch");
    ...
    static void registerRoute(crow::SimpleApp &app, std::string_view path, crow::HTTPMethod method, Wrap wrap)
    {
...
public:
    ...
    static void bindHandler(crow::SimpleApp &app, Handler handler)
    {
        ...
        auto wrap = [handler = std::move(handler)](const crow::request &req, auto... pathArgs) -> crow::response
        {
            ...
            if constexpr (N > 0)
            {
                try
                {
                    path = makePathParams<typename Op::PathParams>(pathArgs...);
                }
                catch (...)
                {
            ...
            auto query = QueryParser::parseParams<typename Op::QueryParams>(req);
            if (!query)
                return crow::response(400, "Bad Request: Invalid or missing query parameters");
            return std::invoke(handler, req, path, *query);
        };
        registerRoute<N>(app, Op::path, toCrowMethod(Op::method), std::move(wrap));
    }
};
